...
See Figure 1 for a graph of the common logarithm function y=log10(x). In this graph, we can see that the logarithm of x-values between 0 and 1 always winds up mapping to y-values between -inf and 0: x-values near 0 have logarithms close to -inf, and x-values close to 1 have logarithms close to 0. Thus, we will select the common logarithm as our function to "blow up" small probability values for comparison.
...
for x in a:
	...
	for c in s:
		if c in 'GC':
			res = res + math.log10(0.5 * x)
		elif c in 'AT':
			res = res + math.log10(0.5 * (1.0 - x))
	print '{:.3f}'.format(res), 
